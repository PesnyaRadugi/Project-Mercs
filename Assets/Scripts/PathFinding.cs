...
public class PathFinding
{
    ...
    public class PathNode
    {
        ...
        public PathNode(int x, int y, int gScore = int.MaxValue, int hScore = 0)
        {
            this.x = x;
            this.y = y;
            this.gScore = gScore;
        ...
        {
            return "x" + x + " y" + y + " g" + gScore + " h" + hScore;
        }
    }
    ...
    public List<PathNode> FindPath(int startX, int startY, int endX, int endY)
    {
        ...
        startNode.gScore = 0;
        startNode.hScore = CalculateDistanceScore(startNode, endNode);
        ...
        while (_openList.Count > 0)
        {
            ...
            foreach (var neighbourNode in GetNeighbours(currentNode))
            {
                ...
                int tentativeGScore =
                    currentNode.gScore +
                    CalculateDistanceScore(currentNode, neighbourNode);
    ...
    private int CalculateDistanceScore(PathNode from, PathNode to)
    {
        ...
        return kDiagonalCost * Mathf.Min(xDistance, yDistance) +
    ...
    private List<PathNode> GetNeighbours(PathNode node)
    {
        ...
        if (HasTile(x, y - 1)) neighbours.Add(GetNode(x, y - 1));
        if (HasTile(x, y + 1)) neighbours.Add(GetNode(x, y + 1));
        if (HasTile(x - 1, y)) neighbours.Add(GetNode(x - 1, y));
    ...
    {
        return _map[new Vector2Int(x, y)];
    }
